      ...
      <div className="w-full max-w-md bg-white h-full overflow-y-auto shadow-2xl border-l border-[var(--border)]">
        <div className="sticky top-0 bg-white border-b border-[var(--border)] px-5 py-4 flex items-center justify-between">
          <div className="min-w-0">
            <div className="font-heading font-semibold text-lg text-[var(--ink)] truncate">{record.name}</div>
            <div className="text-xs text-[var(--ink-3)]">{record.site_name || "Office"} · {record.date}</div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-md hover:bg-[var(--surface-hover)]"><X size={16} /></button>
        </div>

        <div className="p-5 space-y-5">
          <div className="p-3 rounded-lg border border-[var(--border-light)] bg-[var(--surface-2)] grid grid-cols-2 gap-3">
            <div>
              <div className="text-[10px] uppercase tracking-widest font-semibold text-[var(--ink-3)]">Check-In</div>
              <div className="font-mono text-sm">{record.check_in_at ? record.check_in_at.slice(11, 16) : "—"}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-widest font-semibold text-[var(--ink-3)]">Check-Out</div>
              <div className="font-mono text-sm">{record.check_out_at ? record.check_out_at.slice(11, 16) : "—"}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-widest font-semibold text-[var(--ink-3)]">Duration</div>
              <div className="font-mono text-sm">{record.duration_min ? `${record.duration_min} min` : "—"}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase tracking-widest font-semibold text-[var(--ink-3)]">Status</div>
              <div className="text-sm font-medium">{record.status}</div>
            </div>
          </div>

          <div>
            <div className="text-[11px] uppercase tracking-wider font-semibold text-[var(--ink-3)] mb-2">Selfie &amp; Location Check</div>
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm">
                <MapPin size={14} className="text-[var(--ink-3)]" />
                Distance from fence: <span className="font-mono font-semibold">{record.distance_variance_m ?? "—"}m</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Camera size={14} className="text-[var(--ink-3)]" />
                Selfie: {record.selfie_verified ? "Captured" : "None"}
              </div>
              {isAdmin && raw?.check_in_photo && (
                <img src={raw.check_in_photo} alt="Check-in selfie" className="rounded-lg border border-[var(--border)] max-h-40" />
              )}
              {isAdmin && raw && (
              ...
              {!showForm ? (
                ...
                <div className="space-y-2">
                  <select value={reason} onChange={(e) => setReason(e.target.value)}
                    className="w-full px-3 py-1.5 rounded-lg border border-[var(--border)] text-sm" data-testid="regularise-reason">
                    {REASONS.map((r) => <option key={r.value} value={r.value}>{r.label}</option>)}
                  </select>
                  <textarea value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional)"
                    className="w-full px-3 py-1.5 rounded-lg border border-[var(--border)] text-sm" rows={2} />
                  <div className="flex gap-2">
