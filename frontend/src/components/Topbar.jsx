...
export default function Topbar({ title, subtitle, onAdd, addLabel = "New", actions }) {
  return (
    <div className="sticky top-0 z-30 h-16 bg-white/85 backdrop-blur-md border-b border-[var(--border)] flex items-center px-6 gap-4" data-testid="topbar">
      <div className="flex-1 min-w-0">
        <h1 className="font-heading text-[20px] font-semibold text-[var(--ink)] tracking-tight leading-tight truncate">
          {title}
        </h1>
        {subtitle && <div className="text-xs text-[var(--ink-3)] mt-0.5">{subtitle}</div>}
      </div>
