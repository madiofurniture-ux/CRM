...
export default function Inventory() {
  ...
  const save = async () => {
    ...
    try { await api.post("/inventory", { ...form, margin }); toast.success("Item added"); setShow(false); setForm(empty); load(); }
      ...
      <div className="p-6" data-testid="inventory-page">
        ...
        {view === "grid" ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {filtered.map((i) => (
              <div key={i.id} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden hover:shadow-md transition" data-testid={`item-${i.id}`}>
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map((i) => (
                    <tr key={i.id} className="border-t border-[var(--border-light)] hover:bg-[var(--surface-2)]/50">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShow(false)}>
          <div className="bg-white rounded-xl border w-full max-w-xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <h3 className="font-heading font-semibold text-lg">Add Item</h3>
            ...
            <div className="px-5 py-4 border-t flex justify-end gap-2">
              ...
              <button className="btn-primary" onClick={save} data-testid="item-save">Save</button>
            </div>
          </div>
        </div>
      )}
