"""Tests for the Dual-Payment (Cash vs. Bank Transfer + GST) engine and
...
from models import SplitPaymentCreate, BankTransferComponent, CashComponent, PrivacyPinSet, PrivacyPinVerify  # noqa: E402
...
def test_split_payment_computes_gst_and_grand_total():
    async def run():
        payload = SplitPaymentCreate(
            ...
            cash_component=CashComponent(cash_amount=5000),
        )
...
def test_split_payment_cash_only_has_no_gst():
    async def run():
        payload = SplitPaymentCreate(
            project_id="p1", payment_mode="CASH", receipt_date="2026-01-01",
            cash_component=CashComponent(cash_amount=2000),
        )
        doc = await server.create_split_payment(payload, user=ADMIN)
...
def test_split_payment_credits_wallet_when_cash_has_wallet_id():
    async def run():
        await _make_wallet()
        payload = SplitPaymentCreate(
            project_id="p1", payment_mode="CASH", receipt_date="2026-01-01",
            cash_component=CashComponent(cash_amount=500, wallet_id="b1"),
        )
...
def test_split_payment_without_wallet_id_does_not_touch_cashbook():
    async def run():
        ...
        payload = SplitPaymentCreate(
            project_id="p1", payment_mode="CASH", receipt_date="2026-01-01",
            cash_component=CashComponent(cash_amount=500),  # no wallet_id
        )
        await server.create_split_payment(payload, user=ADMIN)
        book = await server.db.cashbooks.find_one({"id": "b1"})
...
def test_list_payments_masks_cash_by_default():
    async def run():
        payload = SplitPaymentCreate(
            project_id="p1", payment_mode="SPLIT", receipt_date="2026-01-01",
            bank_transfer_component=BankTransferComponent(taxable_amount=10000, gst_rate=18.0),
            cash_component=CashComponent(cash_amount=5000, receipt_voucher_no="RV-1"),
        )
        await server.create_split_payment(payload, user=ADMIN)

        masked = await server.list_split_payments(mask_cash=True, user=ADMIN)
        assert masked[0]["cash_component"] is None
        assert masked[0]["bank_transfer_component"]["total_bt_amount"] == 11800  # official figures intact

        unmasked = await server.list_split_payments(mask_cash=False, user=ADMIN)
        assert unmasked[0]["cash_component"]["cash_amount"] == 5000
    asyncio.run(run())


def test_tenant_isolation_on_split_payments():
    async def run():
        payload = SplitPaymentCreate(project_id="p1", payment_mode="CASH", receipt_date="2026-01-01",
                                      cash_component=CashComponent(cash_amount=100))
        await server.create_split_payment(payload, user=ADMIN)
        await server.create_split_payment(payload, user=OTHER_TENANT_ADMIN)

        acme_payments = await server.list_split_payments(mask_cash=False, user=ADMIN)
